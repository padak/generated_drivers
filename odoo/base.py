"""
Base driver interface and utilities for all drivers.

This module provides the abstract base class that all drivers must implement,
defining the standard interface for agents to use.
"""
...
class PaginationStyle(Enum):
    ...
    OFFSET = "offset"  # LIMIT/OFFSET style (SQL, Odoo)
...
class DriverCapabilities:
    ...
    Agents read this to understand available functionality before generating code.
    """
    ...
    query_language: Optional[str] = None  # "SOQL", "SQL", "Odoo Domain", None
...
class BaseDriver(ABC):
    ...
    def from_env(cls, **kwargs) -> "BaseDriver":
        ...
        Example:
            # Reads ODOO_API_URL, ODOO_DATABASE, ODOO_API_KEY from os.environ
    ...
    def list_objects(self) -> List[str]:
        """
        Discover all available objects/models/entities.

        Returns:
            List of object names (e.g., ["res.partner", "sale.order", "res.users"])

        Example:
            objects = client.list_objects()
            # Odoo: ["res.partner", "res.users", "sale.order", ...]
        """
        pass
    ...
    def read(
        ...
        Args:
            query: Query in driver's native language (domain language for Odoo)
    ...
    def read_batched(
        ...
        Args:
            ...
            batch_size: Number of records per batch (default: 80)
    ...
    def close(self):
        ...
        Example:
            client = OdooDriver.from_env()
            try:
                results = client.read("[['active', '=', True]]")
            finally:
                client.close()
        """
        pass
